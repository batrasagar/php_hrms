...
try { $db->query("SELECT 1 FROM tblContractor LIMIT 1"); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $fCompany) {
    ...
    if ($action === 'add') {
        ...
        if ($name === '') $err = 'Contractor name is required.';
        else {
            try {
                $db->prepare("INSERT INTO tblContractor (CompanyId, Name) VALUES (?,?)")->execute([$fCompany, $name]);
                $_SESSION['flash'] = 'Contractor added.';
            } catch (PDOException $e) {
                $err = strpos($e->getMessage(), 'Duplicate') !== false ? 'That contractor already exists.' : 'Could not add contractor.';
            }
        }
    } elseif ($action === 'edit') {
        ...
        if ($id && $name !== '') {
            try {
                $db->prepare("UPDATE tblContractor SET Name=? WHERE id=? AND CompanyId=?")->execute([$name, $id, $fCompany]);
                $_SESSION['flash'] = 'Contractor updated.';
            } catch (PDOException $e) { $err = 'Could not update (duplicate name?).'; }
        }
    } elseif ($action === 'toggle') {
        $db->prepare("UPDATE tblContractor SET IsActive=1-IsActive WHERE id=? AND CompanyId=?")->execute([(int)$_POST['id'], $fCompany]);
        $_SESSION['flash'] = 'Contractor updated.';
    } elseif ($action === 'delete') {
        $db->prepare("DELETE FROM tblContractor WHERE id=? AND CompanyId=?")->execute([(int)$_POST['id'], $fCompany]);
        $_SESSION['flash'] = 'Contractor deleted.';
    }
...
$contractors = [];
if ($fCompany) {
    $s = $db->prepare("SELECT * FROM tblContractor WHERE CompanyId=? ORDER BY Name");
    $s->execute([$fCompany]);
    $contractors = $s->fetchAll();
}
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
  <h5 class="mb-0">Contractor Master</h5>
</div>
...
<div class="row g-3">
  <div class="col-md-4">
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white fw-semibold">Add Contractor</div>
      <div class="card-body">
        <form method="POST">
          <?= csrf_field() ?>
          ...
          <div class="mb-3">
            ...
            <input type="text" name="name" class="form-control" required placeholder="e.g. ABC Manpower" autofocus>
          </div>
  ...
  <div class="col-md-8">
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white fw-semibold">Contractors <span class="badge bg-secondary"><?= count($contractors) ?></span></div>
      <div class="card-body p-0">
        <table class="table table-sm table-hover align-middle mb-0">
          <thead class="table-light"><tr><th>Name</th><th>Status</th><th class="text-end">Actions</th></tr></thead>
          <tbody>
          <?php foreach ($contractors as $d): ?>
          <tr class="<?= $d['IsActive']?'':'text-muted' ?>">
            ...
            <td class="text-end text-nowrap">
              ...
              <form method="POST" class="d-inline" onsubmit="return confirm('Delete this contractor?')"><?= csrf_field() ?><input type="hidden" name="action" value="delete"><input type="hidden" name="company" value="<?= $fCompany ?>"><input type="hidden" name="id" value="<?= $d['id'] ?>">
                <button class="btn btn-sm btn-outline-danger" title="Delete"><i class="bi bi-trash"></i></button>
              </form>
            </td>
          </tr>
          <?php endforeach; ?>
          <?php if (!$contractors): ?><tr><td colspan="3" class="text-center text-muted py-4">No contractors yet. Add one on the left.</td></tr><?php endif; ?>
          </tbody>
        </table>
      </div>
    </div>
